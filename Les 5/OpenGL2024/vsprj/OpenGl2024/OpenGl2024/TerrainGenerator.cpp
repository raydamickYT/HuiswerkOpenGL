...
void TerrainGenerator::generateTerrain(const glm::vec2 currentChunkCord, const glm::vec3 position, const int size, float hScale, float xzScale, glm::vec3 offset, int concurrencyLevel) {
    const int stride = 8;
    int count = size * size;
    ...
    for (int yOffset = -visibleChunks; yOffset <= visibleChunks; ++yOffset)
    {
        for (int xOffset = -visibleChunks; xOffset <= visibleChunks; ++xOffset)
        {
            ...
            else
            {
                ...
                threadPool->enqueue([=]()
                    {
                        generateTerrain(currentChunkCord, chunkWorldPos, chunkSize, 400.0f, xScale, chunkWorldPos, 1);
                    });
            }
        }
    }
